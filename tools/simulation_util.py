...
def rotY(rotz):
    RotZ = np.array([[np.cos(rotz), 0, np.sin(rotz), 0], 
                     [0, 1, 0, 0], 
...
def rotZ(rotz):
    RotZ = np.array([[np.cos(rotz), -np.sin(rotz), 0, 0], 
...
def bbox_IoU(rect1, rect2):
    """ Calculate IoU of two rectangles in centered rectangle format.
        Coordinate system is where x points right, and y points down 
            This is common for pixel locations.
            x1,y1,x2,y2 = left, top, right, bottom

        Note: this can also be used in a coordinate system where y points up.
              Simply provide x1,y1,x2,y2 = left, bottom, right, top

        @param rect1: a rectangle represented as [x1,y1,x2,y2]
        @param rect2: a rectangle represented as [x1,y1,x2,y2]
    """
...
def valid_table_shape(obj_file_name, iou_threshold=0.8):
    ...
    iou = mask_IoU(highest_vertices_xz_rect, table_xz_rect)
...
def imwrite_indexed(filename,array):
    ...
    palette_abspath = './palette.txt' # hard-coded filepath
...
def get_aligned_bbox3D_center(coords):
    """ Compute 3D bounding box center

        @param coords: a dictionary representing 3D bounding box. Has these keys:
                       xmin, xmax, ymin, ymax, zmin, zmax
    """
    return .5 * np.array([coords['xmin']+coords['xmax'], 
                          coords['ymin']+coords['ymax'],
                          coords['zmin']+coords['zmax'],
                         ])

def get_array_of_corners(coords):
    """ Compute matrix of 3D bounding box corners

        @param coords: a dictionary representing 3D bounding box. Has these keys:
                       xmin, xmax, ymin, ymax, zmin, zmax

        @return: a [3 x 8] numpy array of bbox corners
    """
    return np.array([[coords['xmin'], coords['ymin'], coords['zmin']],
                     [coords['xmax'], coords['ymin'], coords['zmin']],
                     [coords['xmin'], coords['ymax'], coords['zmin']],
                     [coords['xmax'], coords['ymax'], coords['zmin']],
                     [coords['xmin'], coords['ymin'], coords['zmax']],
                     [coords['xmax'], coords['ymin'], coords['zmax']],
                     [coords['xmin'], coords['ymax'], coords['zmax']],
                     [coords['xmax'], coords['ymax'], coords['zmax']],
                    ], dtype=np.float32).T
...
def load_shapenetsem_all_models_csv():
    ...
    csv_dir = '/data/ShapeNetSem/ShapeNetSem_csv_files/'
